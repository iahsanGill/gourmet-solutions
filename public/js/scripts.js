...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const closeModalButtons = document.querySelectorAll(".place-order-modal-close");
    ...
    detailsButtons.forEach(button => {
        button.addEventListener("click", () => {
            ...
            const hideButton = button.nextElementSibling;
            if (detailsRow) {
    ...
    hideButtons.forEach(button => {
        button.addEventListener("click", () => {
            ...
            const showButton = button.previousElementSibling;
            if (detailsRow) {
    ...
    orderButtons.forEach(button => {
        button.addEventListener("click", () => {
            ...
            const orderModal = document.getElementById(orderId);
            if (orderModal) {
    ...
    closeModalButtons.forEach(button => {
        button.addEventListener("click", () => {
            const orderModal = button.closest(".place-order-modal");
            if (orderModal) {
    ...
    window.addEventListener("click", (event) => {
        const modals = document.querySelectorAll(".place-order-modal");
...
function openAddModal() {
    document.getElementById('addLunchBoxModal').style.display = 'block';
}

function editLunchBox(id) {
    // Fetch lunch box data
    fetch(`/admin/lunchboxes/${id}/edit`)
        .then(response => response.json())
        .then(data => {
            // Populate the edit modal with the data
            document.getElementById('editLunchBoxModal').style.display = 'block';
            document.getElementById('edit_name').value = data.name;
            document.getElementById('edit_price').value = data.price;
            document.getElementById('edit_ingredients').value = data.ingredients;
            document.getElementById('edit_nutritional_info').value = data.nutritional_info;
            document.getElementById('edit_is_available').checked = data.is_available;
            document.getElementById('edit_form').action = `/admin/lunchboxes/${id}`;
        });
}
